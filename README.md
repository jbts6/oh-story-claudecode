...
网文写作 skill 包，覆盖长篇与短篇网络小说的扫榜、拆文、写作、去AI味、封面图全流程。适配 Codex、Claude Code、OpenClaw。
...
Codex 可通过仓库根目录的 `.codex-plugin/plugin.json` 识别本仓库为插件，插件会暴露 `skills/` 下的全部写作技能。

推荐做法：

1. Fork 或 clone 本仓库。
2. 在 Codex 中安装/启用该本地插件。
3. 在写作项目根目录运行 `/story-setup`，并选择 Codex 目标；Codex 项目说明写入 `AGENTS.md`，项目本地技能副本写入 `.agents/skills/`。

如果只想在当前仓库试用，Codex 已能直接读取根目录 `AGENTS.md` 与 `skills/`。如果要手动给小说项目添加 Codex 说明，可从 [`AGENTS.md.example`](AGENTS.md.example) 复制并按项目名、书名调整。
...
Codex 兼容说明：Claude Code 的 custom agent 由 `.claude/agents/` 注册；Codex 环境下如果没有等价 subagent 工具，技能会按同一角色提示在主会话中执行，并在报告里标明 solo fallback。

## 自动化 Hooks

Claude Code / OpenClaw 中，`/story-setup` 部署后自动生效的 7 个 hook：
...
Codex 兼容说明：这些 hook 脚本仍会随 Claude 目标部署到 `.claude/hooks/`。Codex 目标不会假定 `.claude/settings.local.json` 自动生效；需要时可手动运行对应脚本或按 Codex 当前 hook 配置能力另行注册。
